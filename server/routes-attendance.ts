...
attendanceRouter.post('/attendance/check-in', requireAuth, async (req, res) => {
  const user = req.user!;
  const open = await db.prepare('SELECT id FROM attendance WHERE user_id = ? AND check_out IS NULL').get(user.id);
...
attendanceRouter.post('/attendance/:id/validate', requireAuth, async (req, res) => {
  ...
  await logActivity(user.id, 'attendance', record.id, `validation_${status}`, { userId: record.user_id });
  await notify(record.user_id, 'attendance', `Your attendance on ${record.check_in.slice(0, 10)} was ${status}`, '/portal/attendance');
  res.json({ ok: true });
});
